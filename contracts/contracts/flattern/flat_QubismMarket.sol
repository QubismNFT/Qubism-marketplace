...
/** 
 *  SourceUnit: d:\GitWork\GrahamWorking\QubismNFT\contracts\contracts\QubismMarket.sol
*/
...
////import "./QubismNFT.sol";
////import "./IBEP20.sol";

interface IQubismNFT {
...
contract QubismMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
	...
	uint256 public totalEarning; /* Total Qubism Token */
	...
	function initialize(
        ...
        createCollection("Qubism", "https://ipfs.io/ipfs/QmeiHpFRtNN29KS6eXiNap4nwkphE62FyuV2Mx8gZFeUMe", true);		
	}    
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(QubismNFT).creationCode;
        ...
        IQubismNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection,_token_id) public {
		...
		IQubismNFT nft = IQubismNFT(_collection);        
    ...
    function delist(uint256 _id) external {        
        ...
        IQubismNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].token_id);        
    ...
    function buy(uint256 _id) external ItemExists(_id) {
		...
		// transfer Qubism token to adminAddress
		...
		// transfer Qubism token to owner
		...
		IQubismNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
	...
	modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
        IQubismNFT tokenContract = IQubismNFT(tokenAddress);
